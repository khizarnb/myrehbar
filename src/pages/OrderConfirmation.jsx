        ...
        <div className="border border-[#1a1a1a] p-8 md:p-12">
          ...
          <div className="space-y-4 mb-8">
            {order.order_items.map((item, i) => (
              <div key={i} className="flex justify-between">
                ...
                <p className="font-heading text-sm text-[#E6E2D3]">{item.price_formatted || formatPrice(item.price * item.quantity)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3 pt-6 border-t border-[#1a1a1a]">
            <div className="flex justify-between">
              ...
              <span className="font-body text-sm text-[#E6E2D3]">{formatPrice(order.subtotal)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-mono text-xs tracking-[0.2em] text-[#6B6B6B] uppercase">Shipping</span>
              <span className="font-body text-sm text-[#E6E2D3]">{formatPrice(order.shipping_cost)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-mono text-xs tracking-[0.2em] text-[#6B6B6B] uppercase">Charity Donation</span>
              <span className="font-body text-sm text-[#C4311E]">{formatPrice(order.charity_donation)}</span>
            </div>
            <div className="flex justify-between pt-3 border-t border-[#1a1a1a]">
              ...
              <span className="font-heading text-xl font-black text-[#C4311E]">{order.total_formatted || formatPrice(order.total)}</span>
            </div>
          </div>
        </div>
